...
cd GSE17907
mkdir Data
cd /media/data/jaume/BreastCancer/Data/GSE17907/Data/
wget https://ftp.ncbi.nlm.nih.gov/geo/series/GSE17nnn/GSE17907/suppl/GSE17907_RAW.tar
wget https://ftp.ncbi.nlm.nih.gov/geo/series/GSE17nnn/GSE17907/soft/GSE17907_family.soft.gz
tar -xf GSE17907_RAW.tar
gunzip *.CEL.gz 
...
```{r}
"/media/data/jaume/BreastCancer/Data/GSE17907/Data/"
"/media/data/jaume/BreastCancer/Data/GSE17907/Data/"
GEO_dat  <- getGEO(filename= "/media/data/jaume/BreastCancer/Data/GSE17907/Data/GSE17907_family.soft.gz")
```
...
pCh_Status[pCh_Status == "Breast cancer"] <- "T"
pCh_Status[pCh_Status == "Normal Breast"] <- "NT"

pCh_Age <- pData_GEO_dat$pCh_age_of_diagnosis_.year.
...
pCh_T_Code <- pData_GEO_dat$pCh_pt

pCh_N_Code <- pData_GEO_dat$pCh_pn

pCh_Node <- pData_GEO_dat$pCh_pn
pCh_Node[pCh_Node == "neg"] <- "0"
pCh_Node[pCh_Node == "pos"] <- "1"
pCh_Node[pCh_Node == "--"] <- NA

pCh_ER_Status <- pData_GEO_dat$pCh_er_ihc_status
...
pCh_DFS_T <- pData_GEO_dat$pCh_mfsdel_.month.
pCh_DFS_T[pCh_DFS_T == "--"] <- NA
#pCh_DFS_T <- as.numeric(pCh_DFS_T)/days_to_months

pCh_DFS_E <- pData_GEO_dat$pCh_mfs
pCh_DFS_E[pCh_DFS_E == "--"] <- NA
#pCh_Death_T <- pData_GEO_dat$pCh_os_time
#pCh_Death_T <- as.numeric(pCh_Death_T)/days_to_months

#pCh_Death_E <- pData_GEO_dat$pCh_os_event
...
files_temp <- dir("/media/data/jaume/BreastCancer/Data/GSE17907/Data/",full.names = T)
file.remove(files_temp[grepl("soft.gz",files_temp)])
file.remove(files_temp[grepl(".txt.gz",files_temp)])

rownames(pData_GEO_ft)



#Obtaining md5 for each .CEL file.

datafile <- buildFileTable(rootDir="/media/data/jaume/BreastCancer/Data/GSE17907/Data")
...
directory_files_temp <- dir("/media/data/jaume/BreastCancer/Data/GSE17907/Data",full.names = T)
...
arrayQualityMetrics_All_Groups(GEO_Eset_Raw,"/home/jaume/Projects/BreastMapper/Data/GSE17907/Quality/",var = "pCh_Status",do.logtransform = FALSE) #Already tranformed to log scale.
                               
Tum_Data_QC  <- get_outliers_info(get(load(file="/home/jaume/Projects/BreastMapper/Data/GSE17907/Quality/Results/Pre_Norm/T/Raw_array_quality_metrics_Results.Rda")))
nrow(Tum_Data_QC)
delete_tumors <- names(which(rowSums(Tum_Data_QC[,c(1,3,5)]) == 3))

Cont_Data_QC  <- get_outliers_info(get(load(file="/home/jaume/Projects/BreastMapper/Data/GSE17907/Quality/Results/Pre_Norm/NT/Raw_array_quality_metrics_Results.Rda")))
delete_controls <- names(which(rowSums(Cont_Data_QC[,c(1,3,5)]) == 3))

delete_tmors <- c(delete_tumors,delete_controls)
```
...
pData(GEO_Eset_Norm)$pCh_Batch <- rep("GSE17907",ncol(GEO_Eset_Norm))
...
pData(GEO_Eset_Norm_frma)$pCh_Batch <- rep("GSE17907",ncol(GEO_Eset_Norm_frma))
...
save(file = "/home/jaume/Projects/BreastMapper/Data/GSE17907/Data/GSE17907_Norm.Rda",list_data_norm)
GEO_Eset_Norm <- get(load(file = "/home/jaume/Projects/BreastMapper/Data/GSE17907/Data/GSE17907_Norm.Rda"))
