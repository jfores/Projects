...
cd GSE2603
mkdir Data
cd /media/data/jaume/BreastCancer/Data/GSE2603/Data/
wget https://ftp.ncbi.nlm.nih.gov/geo/series/GSE2nnn/GSE2603/suppl/GSE2603_RAW.tar
wget https://ftp.ncbi.nlm.nih.gov/geo/series/GSE2nnn/GSE2603/soft/GSE2603_family.soft.gz
tar -xf GSE2603_RAW.tar
gunzip *.CEL.gz 
...
```{r}
"/media/data/jaume/BreastCancer/Data/GSE2603/Data/"
"/media/data/jaume/BreastCancer/Data/GSE2603/Data/"
GEO_dat  <- getGEO(filename= "/media/data/jaume/BreastCancer/Data/GSE2603/Data/GSE2603_family.soft.gz")
```
...
#Removing breast cancer cell lines.

pData_GEO_dat <- pData_GEO_dat[!grepl("cell line|MDA-MB",pData_GEO_dat$source_name_ch1,ignore.case = T),]
...
pCh_Age <- pData_GEO_dat$pCh_age_at_dx
...
pCh_Size <-  pData_GEO_dat$pCh_tumor_size_.cm.

  
pCh_ER_Status <- pData_GEO_dat$pCh_path_er_status
pCh_ER_Status[pCh_ER_Status == "N"] <- "0"
pCh_ER_Status[pCh_ER_Status == "P"] <- "1"
#pCh_ER_Status[pCh_ER_Status == "--"] <- NA

pCh_PR_Status <- pData_GEO_dat$pCh_path_pr_status
...
pCh_DFS_T <- pData_GEO_dat$pCh_mfs_.yr.
pCh_DFS_T[pCh_DFS_T == "--"] <- NA
pCh_DFS_T <- as.numeric(pCh_DFS_T)*12

pCh_DFS_E <- pData_GEO_dat$pCh_met_event
...
files_temp <- dir("/media/data/jaume/BreastCancer/Data/GSE2603/Data/",full.names = T)
file.remove(files_temp[grepl("soft.gz",files_temp)])
file.remove(files_temp[grepl("_RAW.tar",files_temp)])
file.remove(files_temp[grepl("EXP.gz",files_temp)])


#Obtaining md5 for each .CEL file.

datafile <- buildFileTable(rootDir="/media/data/jaume/BreastCancer/Data/GSE2603/Data")
datafile <- datafile[gsub(".CEL","",gsub(".*/Data/GSM","GSM",datafile$file)) %in% rownames(pData_GEO_ft),]
...
directory_files_temp <- dir("/media/data/jaume/BreastCancer/Data/GSE2603/Data",full.names = T)
...
```{r}
directory_files_temp <- dir("/media/data/jaume/BreastCancer/Data/GSE2603/Data",full.names = T)
directory_files_temp <- directory_files_temp[gsub(".CEL","",gsub(".*/Data/","",directory_files_temp)) %in% rownames(pData_GEO_ft)]
...
arrayQualityMetrics_All_Groups(GEO_Eset_Raw,"/home/jaume/Projects/BreastMapper/Data/GSE2603/Quality/",var = "pCh_Status",do.logtransform = FALSE) #Already tranformed to log scale.
                               
Tum_Data_QC  <- get_outliers_info(get(load(file="/home/jaume/Projects/BreastMapper/Data/GSE2603/Quality/Results/Pre_Norm/T/Raw_array_quality_metrics_Results.Rda")))
nrow(Tum_Data_QC)
...
pData(GEO_Eset_Norm)$pCh_Batch <- rep("GSE2603",ncol(GEO_Eset_Norm))
...
pData(GEO_Eset_Norm_frma)$pCh_Batch <- rep("GSE2603",ncol(GEO_Eset_Norm_frma))
...
save(file = "/home/jaume/Projects/BreastMapper/Data/GSE2603/Data/GSE2603_Norm.Rda",list_data_norm)
GEO_Eset_Norm <- get(load(file = "/home/jaume/Projects/BreastMapper/Data/GSE2603/Data/GSE2603_Norm.Rda"))
