...
cd GSE65216
mkdir Data
cd /media/data/jaume/BreastCancer/Data/GSE65216/Data/
wget https://ftp.ncbi.nlm.nih.gov/geo/series/GSE65nnn/GSE65216/suppl/GSE65216_RAW.tar
wget https://ftp.ncbi.nlm.nih.gov/geo/series/GSE65nnn/GSE65216/soft/GSE65216_family.soft.gz
tar -xf GSE65216_RAW.tar
gunzip *.CEL.gz 
...
```{r}
"/media/data/jaume/BreastCancer/Data/GSE65216/Data/"
"/media/data/jaume/BreastCancer/Data/GSE65216/Data/"
GEO_dat  <- getGEO(filename= "/media/data/jaume/BreastCancer/Data/GSE65216/Data/GSE65216_family.soft.gz")
```
...
pCh_Status <- pData_GEO_dat$pCh_sample_group
pCh_Status[grepl("Lumi",pCh_Status)] <- "T"
pCh_Status[grepl("Her",pCh_Status)] <- "T"
pCh_Status[grepl("TNBC",pCh_Status)] <- "T"
pCh_Status[grepl("Healthy",pCh_Status)] <- "NT"
pCh_Status[grepl("Cell",pCh_Status)] <- "CL"

pCh_MS_By_DS <- pData_GEO_dat$pCh_sample_group
...
pData_GEO_ft <- pData_GEO_ft[!pData_GEO_ft$pCh_Status == "CL",]


#Removing cel files generated with the B platform.

files_temp <- dir("/media/data/jaume/BreastCancer/Data/GSE65216/Data/",full.names = T)
file.remove(files_temp[grepl("soft.gz",files_temp)])
file.remove(files_temp[grepl("_RAW.tar",files_temp)])

#Removing files not in pheno df.

files_temp <- dir("/media/data/jaume/BreastCancer/Data/GSE65216/Data/",full.names = T)
file.remove(files_temp[!gsub("_.*","",gsub(".*/Data//","",files_temp)) %in% rownames(pData_GEO_ft)])

#Obtaining md5 for each .CEL file.

datafile <- buildFileTable(rootDir="/media/data/jaume/BreastCancer/Data/GSE65216/Data")
...
directory_files_temp <- dir("/media/data/jaume/BreastCancer/Data/GSE65216/Data",full.names = T)
...
arrayQualityMetrics_All_Groups(GEO_Eset_Raw,"/home/jaume/Projects/BreastMapper/Data/GSE65216/Quality/",var = "pCh_Status",do.logtransform = FALSE) #Already tranformed to log scale.
                               
Tum_Data_QC  <- get_outliers_info(get(load(file="/home/jaume/Projects/BreastMapper/Data/GSE65216/Quality/Results/Pre_Norm/T/Raw_array_quality_metrics_Results.Rda")))
nrow(Tum_Data_QC)

delete_tumors <- names(which(rowSums(Tum_Data_QC[,c(1,3,5)]) == 3))


Cont_Data_QC  <- get_outliers_info(get(load(file="/home/jaume/Projects/BreastMapper/Data/GSE65216/Quality/Results/Pre_Norm/NT/Raw_array_quality_metrics_Results.Rda")))
delete_controls <- names(which(rowSums(Cont_Data_QC[,c(1,3,5)]) == 3))

delete_tumors <- c(delete_tumors,delete_controls)
```
...
pData(GEO_Eset_Norm)$pCh_Batch <- rep("GSE65216",ncol(GEO_Eset_Norm))
...
pData(GEO_Eset_Norm_frma)$pCh_Batch <- rep("GSE65216",ncol(GEO_Eset_Norm_frma))
...
save(file = "/home/jaume/Projects/BreastMapper/Data/GSE65216/Data/GSE65216_Norm.Rda",list_data_norm)
GEO_Eset_Norm <- get(load(file = "/home/jaume/Projects/BreastMapper/Data/GSE65216/Data/GSE65216_Norm.Rda"))
table(exprs(GEO_Eset_Norm[[1]]) == exprs(GEO_Eset_Norm[[2]]))
table(GEO_Eset_Norm[[1]]$pCh_Status)
```
