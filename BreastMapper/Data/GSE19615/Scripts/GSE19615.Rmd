...
cd GSE19615
mkdir Data
cd /media/data/jaume/BreastCancer/Data/GSE19615/Data/
wget https://ftp.ncbi.nlm.nih.gov/geo/series/GSE19nnn/GSE19615/suppl/GSE19615_RAW.tar
wget https://ftp.ncbi.nlm.nih.gov/geo/series/GSE19nnn/GSE19615/soft/GSE19615_family.soft.gz
tar -xf GSE19615_RAW.tar
gunzip *.CEL.gz 
...
```{r}
"/media/data/jaume/BreastCancer/Data/GSE19615/Data/"
"/media/data/jaume/BreastCancer/Data/GSE19615/Data/"
GEO_dat  <- getGEO(filename= "/media/data/jaume/BreastCancer/Data/GSE19615/Data/GSE19615_family.soft.gz")
```
...
pCh_DFS_T_A <- pData_GEO_dat$pCh_distant_recurrence_free_survival_.mo.
pCh_DFS_T_B <- pData_GEO_dat$pCh_time_of_followup_.mo.
pCh_DFS_T <- c()
for(i in 1:length(pCh_DFS_T_A)){
  if(pCh_DFS_T_A[i] != "NA"){
    pCh_DFS_T <- c(pCh_DFS_T,pCh_DFS_T_A[i])
  }else{
    pCh_DFS_T <- c(pCh_DFS_T,pCh_DFS_T_B[i])
  }
}

#pCh_DFS_T_comments <- rep("It is distant metastais free survival expressed in months",nrow(pData_GEO_dat))

pCh_DFS_E <- pData_GEO_dat$pCh_distant_recur_.yn.
pCh_DFS_E[pCh_DFS_E == "Y"] <- "1"
pCh_DFS_E[pCh_DFS_E == "N"] <- "0"

pCh_Node <- pData_GEO_dat$pCh_lymph_nodes
pCh_Node[grepl("pos",pCh_Node)] <- "1"
pCh_Node[grepl("neg",pCh_Node)] <- "0"
pCh_Node_comments <- rep("1 if affected nodes and 0 if not",nrow(pData_GEO_dat))

pCh_Grade <- pData_GEO_dat$pCh_grade_.modified._bloom._richardson.
pCh_Grade[pCh_Grade == "I"] <- "1"
pCh_Grade[pCh_Grade == "II"] <- "2"
pCh_Grade[pCh_Grade == "III"] <- "3"

pCh_Tumor_Size <- pData_GEO_dat$pCh_tumor_size_.cm.
pCh_Tumor_Size_comment <- rep("Expressed in cm",nrow(pData_GEO_dat))

pCh_ER_Status <- pData_GEO_dat$pCh_er
pCh_ER_Status[grepl("pos",pCh_ER_Status)] <- "1"
pCh_ER_Status[grepl("neg",pCh_ER_Status)] <- "0"

pCh_HER2_Status <- pData_GEO_dat$pCh_her.2
pCh_HER2_Status[grepl("pos",pCh_HER2_Status)] <- "1"
pCh_HER2_Status[grepl("neg",pCh_HER2_Status)] <- "0"

pCh_PR_Status <- pData_GEO_dat$pCh_pr
pCh_PR_Status[grepl("pos",pCh_PR_Status)] <- "1"
pCh_PR_Status[grepl("neg",pCh_PR_Status)] <- "0"

pCh_Age <- pData_GEO_dat$pCh_age

pCh_Histology <- pData_GEO_dat$pCh_histology_type
...
files_temp <- dir("/media/data/jaume/BreastCancer/Data/GSE19615/Data/",full.names = T)
...
datafile <- buildFileTable(rootDir="/media/data/jaume/BreastCancer/Data/GSE19615/Data")
...
directory_files_temp <- dir("/media/data/jaume/BreastCancer/Data/GSE19615/Data",full.names = T)
...
arrayQualityMetrics_All_Groups(GEO_Eset_Raw,"/home/jaume/Projects/BreastMapper/Data/GSE19615/Quality/",var = "pCh_Status",do.logtransform = FALSE) #Already tranformed to log scale.
                               
Tum_Data_QC  <- get_outliers_info(get(load(file="/home/jaume/Projects/BreastMapper/Data/GSE19615/Quality/Results/Pre_Norm/T/Raw_array_quality_metrics_Results.Rda")))
nrow(Tum_Data_QC)
...
pData(GEO_Eset_Norm)$pCh_Batch <- rep("GSE19615",ncol(GEO_Eset_Norm))
...
pData(GEO_Eset_Norm_frma)$pCh_Batch <- rep("GSE19615",ncol(GEO_Eset_Norm_frma))
...
save(file = "/home/jaume/Projects/BreastMapper/Data/GSE19615/Data/GSE19615_Norm.Rda",list_data_norm)
GEO_Eset_Norm <- get(load(file = "/home/jaume/Projects/BreastMapper/Data/GSE19615/Data/GSE19615_Norm.Rda"))
