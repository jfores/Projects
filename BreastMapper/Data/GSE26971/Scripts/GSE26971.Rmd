...
cd GSE26971
mkdir Data
cd /media/data/jaume/BreastCancer/Data/GSE26971/Data/
wget https://ftp.ncbi.nlm.nih.gov/geo/series/GSE26nnn/GSE26971/suppl/GSE26971_RAW.tar
wget https://ftp.ncbi.nlm.nih.gov/geo/series/GSE26nnn/GSE26971/soft/GSE26971_family.soft.gz
tar -xf GSE26971_RAW.tar
gunzip *.CEL.gz 
gunzip *.cel.gz 
dir()
files_temp <- dir("/media/data/jaume/BreastCancer/Data/GSE26971/Data/",full.names = T)
file.remove(files_temp[grepl(".CEL$",files_temp)])
file.remove(files_temp[grepl(".md5$",files_temp)])
file.remove(files_temp[grepl(".cel.gz$|CEL.gz$",files_temp)])
```
...
```{r}
"/media/data/jaume/BreastCancer/Data/GSE26971/Data/"
"/media/data/jaume/BreastCancer/Data/GSE26971/Data/"
GEO_dat  <- getGEO(filename= "/media/data/jaume/BreastCancer/Data/GSE26971/Data/GSE26971_family.soft.gz")
```
...
get_met_data <- function(data,string){
  data <- Meta(data)
  data <- data$characteristics_ch1
  data <- data[grepl(string,data)]
  return(data)
}

#Extrat extra infor

temp_GSMList <- GSMList(GEO_dat)
Meta(temp_GSMList[[1]])$characteristics_ch1
temp_out <- lapply(temp_GSMList,get_met_data,string = "1: yes")
data_frame_out_Met <- data.frame(names(unlist(temp_out)),unlist(temp_out))
data_frame_out_Met$unlist.temp_out. <- gsub(".*: ","",data_frame_out_Met$unlist.temp_out.)
colnames(data_frame_out_Met) <- paste("Met_",colnames(data_frame_out_Met),sep="")

temp_out <- lapply(temp_GSMList,get_met_data,string = "positive lymph")
data_frame_out_Lymphh <- data.frame(names(unlist(temp_out)),unlist(temp_out))
data_frame_out_Lymphh$unlist.temp_out. <- gsub("positive lymph nodes: ","",data_frame_out_Lymphh$unlist.temp_out.)
data_frame_out_Lymphh$unlist.temp_out. <- ifelse(data_frame_out_Lymphh$unlist.temp_out. == "none","0","1")
colnames(data_frame_out_Lymphh) <- paste("Lymph_",colnames(data_frame_out_Lymphh),sep="")

rownames(pData_GEO_dat) <- pData_GEO_dat$geo_accession

pData_GEO_dat <- merge(pData_GEO_dat,data_frame_out_Met,by="row.names",all.x = T)
rownames(pData_GEO_dat) <- pData_GEO_dat$Row.names
pData_GEO_dat <- pData_GEO_dat[,-1]
pData_GEO_dat <- merge(pData_GEO_dat,data_frame_out_Lymphh,by="row.names",all.x = T)
View(pData_GEO_dat)
rownames(pData_GEO_dat) <- pData_GEO_dat$Row.names
pData_GEO_dat <- pData_GEO_dat[,-1]
...
#pCh_Age <- pData_GEO_dat$pCh_age_at_diagnosis
...
#pCh_T_Code <- pData_GEO_dat$pCh_t_stage

#pCh_N_Code <- pData_GEO_dat$pCh_n_stage
...
pCh_DFS_T <- pData_GEO_dat$pCh_metastasis_free_interval_.months.

#a_temp <- pData_GEO_dat$pCh_regional_relapse
#b_temp <- pData_GEO_dat$pCh_event_metastasis
#b_temp[a_temp=="1"] <- 1

pCh_DFS_E <- pData_GEO_dat$Met_unlist.temp_out.

View(pData_GEO_dat)
pCh_Node <- pData_GEO_dat$Lymph_unlist.temp_out.
#pCh_Node[pCh_Node == "2"] <- "1"
#pCh_Node[pCh_Node == "3"] <- "1"
#pCh_Node[pCh_Node == "--"] <- NA

pCh_Size <-  pData_GEO_dat$pCh_tumor_size
  
#pCh_ER_Status <- pData_GEO_dat$pCh_er_status
#pCh_ER_Status[pCh_ER_Status == "neg"] <- "0"
#pCh_ER_Status[pCh_ER_Status == "pos"] <- "1"
#pCh_ER_Status[pCh_ER_Status == "--"] <- NA

#pCh_PR_Status <- pData_GEO_dat$pCh_pr_ihc_status
#pCh_PR_Status[pCh_PR_Status == "neg"] <- "0"
#pCh_PR_Status[pCh_PR_Status == "pos"] <- "1"
#pCh_PR_Status[pCh_PR_Status == "--"] <- NA

#pCh_ERBB2_Status <- pData_GEO_dat$pCh_erbb2_ihc_status
#pCh_ERBB2_Status[pCh_ERBB2_Status == "neg"] <- "0"
#pCh_ERBB2_Status[pCh_ERBB2_Status == "pos"] <- "1"
...
datafile <- buildFileTable(rootDir="/media/data/jaume/BreastCancer/Data/GSE26971/Data")
result <-validateFile(datafile)
result <- result$filetable
rownames(result) <- gsub("_.*","",gsub(".CEL|.cel","",gsub(".*/Data/","",result$file)))
...
directory_files_temp <- dir("/media/data/jaume/BreastCancer/Data/GSE26971/Data",full.names = T)
directory_files_temp_to_delete <- directory_files_temp[grepl(".CEL.md5$|.cel.md5$",directory_files_temp)]
...
cols_eDat <- gsub("_.*","",gsub(".CEL|.cel","",cols_eDat))
...
colnames(GEO_Eset_Raw) <- gsub("_.*","",gsub(".CEL|.cel","",colnames(GEO_Eset_Raw)))
...
arrayQualityMetrics_All_Groups(GEO_Eset_Raw,"/home/jaume/Projects/BreastMapper/Data/GSE26971/Quality/",var = "pCh_Status",do.logtransform = FALSE) #Already tranformed to log scale.
                               
Tum_Data_QC  <- get_outliers_info(get(load(file="/home/jaume/Projects/BreastMapper/Data/GSE26971/Quality/Results/Pre_Norm/T/Raw_array_quality_metrics_Results.Rda")))
nrow(Tum_Data_QC)
...
pData(GEO_Eset_Norm)$pCh_Batch <- rep("GSE26971",ncol(GEO_Eset_Norm))
...
pData(GEO_Eset_Norm_frma)$pCh_Batch <- rep("GSE26971",ncol(GEO_Eset_Norm_frma))
...
save(file = "/home/jaume/Projects/BreastMapper/Data/GSE26971/Data/GSE26971_Norm.Rda",list_data_norm)
GEO_Eset_Norm <- get(load(file = "/home/jaume/Projects/BreastMapper/Data/GSE26971/Data/GSE26971_Norm.Rda"))
