...
cd GSE45255
mkdir Data
cd /media/data/jaume/BreastCancer/Data/GSE45255/Data/
wget https://ftp.ncbi.nlm.nih.gov/geo/series/GSE45nnn/GSE45255/suppl/GSE45255_RAW.tar
wget https://ftp.ncbi.nlm.nih.gov/geo/series/GSE45nnn/GSE45255/soft/GSE45255_family.soft.gz
tar -xf GSE45255_RAW.tar
gunzip *.CEL.gz 
...
```{r}
"/media/data/jaume/BreastCancer/Data/GSE45255/Data/"
"/media/data/jaume/BreastCancer/Data/GSE45255/Data/"
GEO_dat  <- getGEO(filename= "/media/data/jaume/BreastCancer/Data/GSE45255/Data/GSE45255_family.soft.gz")
```
...
pCh_Age <- pData_GEO_dat$pCh_patient_age
...
pCh_Node <- pData_GEO_dat$pCh_ln_status
pCh_Node[pCh_Node == "LN-"] <- "0"
pCh_Node[pCh_Node == "LN+"] <- "1"
#pCh_Node[pCh_Node == "--"] <- NA

pCh_Size <-  pData_GEO_dat$pCh_size_.mm.
...
pCh_PR_Status <- pData_GEO_dat$pCh_pgr_status
pCh_PR_Status[pCh_PR_Status == "PgR-"] <- "0"
pCh_PR_Status[pCh_PR_Status == "PgR+"] <- "1"
...
pCh_HER2_Status[pCh_HER2_Status == "He-"] <- "0"
pCh_HER2_Status[pCh_HER2_Status == "He+"] <- "1"
#pCh_Treatment <- pData_GEO_dat$pCh_endocrine_therapy

pCh_Grade <- pData_GEO_dat$pCh_histological_grade

#pCh_HER2_Fish <- pData_GEO_dat$pCh_her2fish

pCh_DFS_T <- pData_GEO_dat$pCh_dfs_time
#pCh_DFS_T[pCh_DFS_T == "--"] <- NA
pCh_DFS_T <- as.numeric(pCh_DFS_T)*12

pCh_DFS_E <- pData_GEO_dat$pCh_dfs_event_.defined_as_any_type_of_recurrence_or_death_from_breast_cancer.
#pCh_DFS_E[pCh_DFS_E == "--"] <- NA

pCh_Death_T <- pData_GEO_dat$pCh_DSS_time
#pCh_Death_T <- as.numeric(pCh_Death_T)*12
pCh_Death_T <- as.numeric(pCh_Death_T)*12

pCh_Death_E <- pData_GEO_dat$pCh_dss_event_.defined_as_death_from_breast_cancer.
...
files_temp <- dir("/media/data/jaume/BreastCancer/Data/GSE45255/Data/",full.names = T)
...
datafile <- buildFileTable(rootDir="/media/data/jaume/BreastCancer/Data/GSE45255/Data")
...
directory_files_temp <- dir("/media/data/jaume/BreastCancer/Data/GSE45255/Data",full.names = T)
...
arrayQualityMetrics_All_Groups(GEO_Eset_Raw,"/home/jaume/Projects/BreastMapper/Data/GSE45255/Quality/",var = "pCh_Status",do.logtransform = FALSE) #Already tranformed to log scale.
                               
Tum_Data_QC  <- get_outliers_info(get(load(file="/home/jaume/Projects/BreastMapper/Data/GSE45255/Quality/Results/Pre_Norm/T/Raw_array_quality_metrics_Results.Rda")))
nrow(Tum_Data_QC)
...
pData(GEO_Eset_Norm)$pCh_Batch <- rep("GSE45255",ncol(GEO_Eset_Norm))
...
pData(GEO_Eset_Norm_frma)$pCh_Batch <- rep("GSE45255",ncol(GEO_Eset_Norm_frma))
...
save(file = "/home/jaume/Projects/BreastMapper/Data/GSE45255/Data/GSE45255_Norm.Rda",list_data_norm)
GEO_Eset_Norm <- get(load(file = "/home/jaume/Projects/BreastMapper/Data/GSE45255/Data/GSE45255_Norm.Rda"))
