...
#cd /media/data/jaume/BreastCancer/Data/GSE1456/Data/
#wget https://ftp.ncbi.nlm.nih.gov/geo/series/GSE1nnn/GSE1456/suppl/GSE1456_RAW.tar
#wget https://ftp.ncbi.nlm.nih.gov/geo/series/GSE1nnn/GSE1456/soft/GSE1456_family.soft.gz
#gunzip *.CEL.gz (Unzip all gz files in a directory)
```
...
```{r}
"/media/data/jaume/BreastCancer/Data/GSE1456/Data/"

GEO_dat  <- getGEO(filename= "/media/data/jaume/BreastCancer/Data/GSE1456/Data/GSE1456_family.soft.gz")
```
...
pCh_DFS_T <- as.character(as.numeric(pData_GEO_dat$pCh_SURV_RELAPSE)*12) #Relaps in years transform to months.

pCh_DFS_T_comments <- rep("It is distant metastais free survival expressed in years but we transformed it to months",nrow(pData_GEO_dat))
pCh_DFS_E <- pData_GEO_dat$pCh_RELAPSE #Relapse event information.

pCh_Death_T <- as.character(as.numeric(pData_GEO_dat$pCh_SURV_DEATH)*12)

pCh_Death_T_comments <- rep("It is time to death in years but we transformed it to months",nrow(pData_GEO_dat))

pCh_Death_E <- pData_GEO_dat$pCh_DEATH

pCh_Subtype <- pData_GEO_dat$pCh_SUBTYPE

pCh_Elston <- pData_GEO_dat$pCh_ELSTON
...
files_temp <- dir("/media/data/jaume/BreastCancer/Data/GSE1456/Data/",full.names = T)
delete_files <- !gsub(".CEL.gz","",gsub(".*/","",files_temp)) %in% rownames(pData_GEO_ft)
delete_files_names <- files_temp[delete_files]
for(i in 1:length(delete_files_names)){
  file.remove(delete_files_names[i])
}

#Obtaining md5 for each .CEL file.

directory_files_temp <- dir("/media/data/jaume/BreastCancer/Data/GSE1456/Data",full.names = T)
directory_files_temp_to_delete <- directory_files_temp[grepl(".CEL.gz.md5$",directory_files_temp)]
for(i in 1:length(directory_files_temp_to_delete)){
  file.remove(directory_files_temp_to_delete[i])
}

datafile<-buildFileTable(rootDir="/media/data/jaume/BreastCancer/Data/GSE1456/Data")
...
arrayQualityMetrics_All_Groups(GEO_Eset_Raw,"/home/jaume/Projects/BreastMapper/Data/GSE1456/Quality/",var = "pCh_Status",do.logtransform = FALSE) #Already tranformed to log scale.
                               
Tum_Data_QC  <- get_outliers_info(get(load(file="/home/jaume/Projects/BreastMapper/Data/GSE1456/Quality/Results/Pre_Norm/T/Raw_array_quality_metrics_Results.Rda")))
dim(Tum_Data_QC)
...
pData(GEO_Eset_Norm)$pCh_Batch <- rep("GSE1456",ncol(GEO_Eset_Norm))
...
pData(GEO_Eset_Norm_frma)$pCh_Batch <- rep("GSE1456",ncol(GEO_Eset_Norm_frma))
```

```{r}
list_data_norm <- list(GEO_Eset_Norm,GEO_Eset_Norm_frma)
save(file = "/home/jaume/Projects/BreastMapper/Data/GSE1456/Data/GSE1456_Norm.Rda",list_data_norm)
GEO_Eset_Norm <- get(load(file = "/home/jaume/Projects/BreastMapper/Data/GSE1456/Data/GSE1456_Norm.Rda"))
pData(GEO_Eset_Norm[[1]])$pCh_Status
table(exprs(GEO_Eset_Norm[[1]]) == exprs(GEO_Eset_Norm[[2]]))
library(limma)
plot(prcomp(exprs(GEO_Eset_Norm[[1]])))
pca.plot <- autoplot(prcomp(exprs()), data = pilots, colour = 'Group')
pca.plot
```
